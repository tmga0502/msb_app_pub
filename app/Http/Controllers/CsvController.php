    ...
    if($bank_number == '17' || $bank_number == '19'){
      ...
      foreach ($file as $row) {
        ...
        if ($row_count > 1) {
          ...
          array_push($array, $csvimport_array);// つくった配列の箱($array)に追加
        }
        $row_count++;
      }

      Csv::insert($array);//バルクインサート
      //コミッションtebleも作成
      $inportCsv = Csv::where('year', '=', $year_csv)->where('month', '=', $month_csv)->get();// 登録したCsvテーブルを取得
      $dates_instance = new Commission\GetDatas;// jsonArrayの作成
      $userList = $dates_instance->userList();
      $jsonArray = [];
      foreach ($userList as $list) {
          $jsonArray[$list] = ['rate' => 0, 'price' => 0];
      }
      foreach ($inportCsv as $in) {// commissionTableのcreate
